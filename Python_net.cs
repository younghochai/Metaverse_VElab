    ...
    void Update()
    {
        ...
        if (socketReady)
        {
            if (stream.DataAvailable)
            {
                ...
                //string strTemp = Regex.Replace(msg, @"\d", "");//문자 추출
                //string strTemp2 = Regex.Replace(msg, @"\D", "");//숫자 추출
                ...
                {
                    //Debug.Log("word : " + word);
                    string strTemp = Regex.Replace(word, @"\d", "");//문자 추출
                    string strTemp2 = Regex.Replace(word, @"\D", "");//숫자 추출
