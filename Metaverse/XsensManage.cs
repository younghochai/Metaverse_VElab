...
public class XsensManage : MonoBehaviour
{
    ...
    // sensing device save file
    private List<Quaternion> init_quat_data;
    bool save_state = false;
    List<List<Quaternion>> save_quat_list = new List<List<Quaternion>>();
    int file_count = 0;
    bool new_Data_com = false;
    ...
    void Start()
    {
        sensors.Add(new XsensDevice("00B43808"));
        sensors.Add(new XsensDevice("00B438AE"));
        sensors.Add(new XsensDevice("00B438C7"));
        sensors.Add(new XsensDevice("00B43923"));
        sensors.Add(new XsensDevice("00B43926"));
        sensors.Add(new XsensDevice("00B4391F"));
        //TestFuncion();
    ...
    void save_init_quat()
    {
        ...
        init_quat_data = new List<Quaternion>();
        ...
        for (int i = 0; i < sensors.Count; i++)
        {
            init_quat_data.Add(new Quaternion((float)sensors[i].QuatX, (float)sensors[i].QuatY, (float)sensors[i].QuatZ, (float)sensors[i].QuatW));
        }

        for (int i = 0; i < sensors.Count; i++)
        {
            save_quat_list.Add(new List<Quaternion>());
            save_quat_list[i].Add(init_quat_data[i]);
        }
    }
    ...
    void reset_init_quat()
    {
        ...
        for (int i = 0; i < init_quat_data.Count(); i++)
        {
            save_quat_list.Add(new List<Quaternion>());
            save_quat_list[i].Add(init_quat_data[i]);
        }
    ...
    void saving_quat()
    {
        ...
        for (int i = 0; i < init_quat_data.Count(); i++)
        {
            ...
            save_quat_list[i].Add(new Quaternion((float)sensors[i].QuatX, (float)sensors[i].QuatY, (float)sensors[i].QuatZ, (float)sensors[i].QuatW));
    ...
    void write_csv_file()
    {
...

        {
            ...
            file.WriteLine(sensors[0].deviceNum + ", x, y, z," + sensors[1].deviceNum + ", x, y, z," + sensors[2].deviceNum + ", x, y, z," + sensors[3].deviceNum + ", x, y, z," + sensors[4].deviceNum + ", x, y, z," + sensors[5].deviceNum + ", x, y, z");
            ...
            for (int frame = 0; frame < save_quat_list[0].Count(); frame++)
            {
                ...
                for (int joint = 0; joint < init_quat_data.Count(); joint++)
                {
                    ...
                    if (joint == init_quat_data.Count() - 1)
